...
from package.chain.extract_tasks import extract_tasks_from_sticky_notes
from package.common import get_logger, get_settings
from package.util.miro.sticky_notes import get_random_color, get_selected_sticky_notes
...
def post_extract_tasks(request: TaskRequest) -> TaskResponse:
    ...
    try:
        ...
        sticky_notes = get_selected_sticky_notes(
            miro, request.board_id, request.item_ids
        )
        ...
        for i, task in enumerate(extracted_tasks.tasks):
            ...
            # タスク付箋を作成
            task_sticky_note_data = {
                "data": {"content": task, "shape": "square"},
                "style": {"fillColor": "#FFE066"},  # タスクは黄色に統一
                "position": {"x": x, "y": y},
            }
            ...
            created_task = miro.api.create_sticky_note_item(
                ...
                sticky_note_create_request=StickyNoteCreateRequest(
                    ...
                    style=StickyNoteStyle(fillColor=get_random_color()),  # タスクは黄色
        ...
        raise HTTPException(
            status_code=500,
            detail=f"タスク切り出し中にエラーが発生しました: {e!s}",
        )
